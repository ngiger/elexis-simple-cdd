...
then
 ...
 cmd="$cmd  --vlan0 rtl8139=${mac_id} -smp 1 --bootdisk ide0 --ide0 32 --ostype l26 --memory 512 --onboot yes --format qcow2 --cache writeback"
 cmd="$cmd --smp 1 --memory 2024 "
#  echo "memory: 512" >>  ${fName}
#  echo "description: elexis-admin VM ${vmId} (main server, db, wiki) with puppet. MAC-id is ${mac_id}"  >> ${fName}
  cmd="$cmd --description: elexis-admin VM ${vmId} (main server, db, wiki) with puppet. MAC-id is ${mac_id}"
  echo $cmd
  $cmd
